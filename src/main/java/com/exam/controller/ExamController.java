 ...
 * 考试接口 —— 学生端。
 *
 * <h3>为什么整类限定 STUDENT 角色</h3>
 * <p>
 * 教师和管理员不该参加考试。这个限制看起来理所当然，
 * 但值得说明它<b>不只是"业务上不合适"</b>：
 * <p>
 * 如果放开，教师账号就能开考。而开考会在 {@code exam_record} 里写一条记录，
 * 这条记录会被算进"本场考试的参与人数"、会出现在成绩统计里、
 * 也会占用 {@code uk_paper_user} 这个唯一索引的位置。
 * <p>
 * 结果是：教师的"试考"污染了真实的考试数据，而且<b>没法区分哪些是试考</b>——
 * 因为记录本身长得一模一样。要清理就得靠人工比对，还容易误删真数据。
 * <p>
 * 所以这不是"权限收得紧一点更保险"，而是<b>从源头保证数据的语义纯净</b>：
 * 一条 exam_record 就代表一个真实考生。
 *
 * <h3>配套说明：本类没有"教师查全部考试"的接口</h3>
 * <p>
 * 学生只能看自己的考试（{@code myRecords} + 归属校验）。
 * 教师批阅、班级统计属于阅卷端，是独立的一块，不在 Day 5 范围内。
 */
...
public class ExamController {
    ...
    @Operation(summary = "获取试卷（断点续考/刷新）", description = """
            ...
            因为作答状态存在服务端，关掉浏览器、换一台电脑重新登录，
            调本接口就能完整恢复现场 —— 这是本系统相比"存在浏览器本地"的核心优势。
            """)
